...
// 添加阈值常量
#define MIN_PARALLEL_SIZE 10000 // 小于此数量的PT用CPU处理
...
void PriorityQueue::Generate(PT pt) {
    ...
    if (N < MIN_PARALLEL_SIZE) {
        if (is_single_segment) {
            ...
            for (int i = 0; i < N; ++i) {
                guesses.push_back(fixed_part + a->ordered_values[i]);
            }
        }
        total_guesses += N;
        return;
    ...
    try {
        ...
        // 使用普通内存而不是pinned memory
        char* h_flatten = new char[total_chars];
        if (!h_flatten) throw runtime_error("Failed to allocate host memory");

        // 分块处理数据
        const int CHUNK_SIZE = 100000; // 每次处理CHUNK_SIZE条数据
        int processed = 0;

        while (processed < N) {
            int current_chunk = min(CHUNK_SIZE, N - processed);
            size_t chunk_chars = 0;
            
            // 计算当前块需要的字符数
            for (int i = processed; i < processed + current_chunk; ++i) {
                chunk_chars += lengths[i];
            }

            // 为当前块分配设备内存
            char *d_chunk_flatten = nullptr;
            char *d_chunk_output = nullptr;
            int *d_chunk_lengths = nullptr;
            int *d_chunk_offsets = nullptr;
            char *d_fixed = nullptr;  // 全局变量

            cudaMalloc(&d_chunk_flatten, chunk_chars);
            cudaMalloc(&d_chunk_output, current_chunk * MAX_PW_LEN);
            cudaMalloc(&d_chunk_lengths, current_chunk * sizeof(int));
            cudaMalloc(&d_chunk_offsets, current_chunk * sizeof(int));

            // 准备当前块的数据
            vector<int> chunk_offsets(current_chunk);
            size_t offset = 0;
            for (int i = 0; i < current_chunk; ++i) {
                chunk_offsets[i] = offset;
                memcpy(h_flatten + offset, values[processed + i].data(), lengths[processed + i]);
                offset += lengths[processed + i];
            }

            // 拷贝数据到设备
            cudaMemcpy(d_chunk_flatten, h_flatten, chunk_chars, cudaMemcpyHostToDevice);
            cudaMemcpy(d_chunk_lengths, &lengths[processed], current_chunk * sizeof(int), cudaMemcpyHostToDevice);
            cudaMemcpy(d_chunk_offsets, chunk_offsets.data(), current_chunk * sizeof(int), cudaMemcpyHostToDevice);

            // 配置kernel
            dim3 block(256);
            dim3 grid((current_chunk + block.x - 1) / block.x);

            if (is_single_segment) {
                generate_kernel_single<<<grid, block>>>(
                    d_chunk_flatten, d_chunk_lengths, d_chunk_offsets,
                    d_chunk_output, MAX_PW_LEN, current_chunk
                );
            } else {
                if (fixed_part.size() > 0) {
            ...
            // 获取结果
            char* h_output = new char[current_chunk * MAX_PW_LEN];
            cudaMemcpy(h_output, d_chunk_output, current_chunk * MAX_PW_LEN, cudaMemcpyDeviceToHost);
